...
def best_flow1(G, F, V):
    ...
    for t in range(30):
        ...
        for total_flow, valve, open_valves in sorted(states, reverse=True)[:10000]:  # check 10k best states
...
def best_flow2(G, F, V):
    ...
    for t in range(26):
        next_states = set()
        print(f"searching at t={t}, nstates={len(states)}")
        for total_flow, valves, open_valves in sorted(states, reverse=True)[:10000]:  # check 10k best states
            total_flow += sum([F[ov] for ov in open_valves])
            vs = valves.members()
